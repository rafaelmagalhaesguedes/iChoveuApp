...
export const searchCities = async (term) => {
  const response = await fetch(`https://api.weatherapi.com/v1/search.json?lang=pt&key=${APIKEY}&q=${term}`);
...
export const getWeatherByCity = async (url) => {
  const response = await fetch(`https://api.weatherapi.com/v1/current.json?lang=pt&key=${APIKEY}&q=${url}`);
  const data = await response.json();
  const weatherInfo = {
...
export const fetchForecastData = async (cityUrl) => {
  const urlApi = `https://api.weatherapi.com/v1/forecast.json?lang=pt&key=${APIKEY}&q=${cityUrl}&days=${FORECAST}`;
